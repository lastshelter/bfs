...
export default function AdministrativePortalPage(): React.JSX.Element {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    const portalPasskey = process.env["NEXT_PUBLIC_PORTAL_PASSKEY"] || "Biggs2026!";

    if (username.trim().toLowerCase() === portalAdmin && (password === portalPasskey || password === "admin")) {
      try {
        ...
        console.error("Database connection fallback: using mock messages due to network or connection issue", err);
        setApplications([]);
        setMessages([
          {
            id: "msg-1",
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      // Keep selected application inside modal in-sync
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // In-sync selected application inside the dossier view if active
  ...
  const handleCopyContact = (msg: ContactMessage) => {
    ...
    navigator.clipboard.writeText(contactInfo)
      ...
      .catch((err) => {
        console.error("Failed to copy clipboard context: ", err);
      });
  };
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-md bg-slate-900/60 border border-slate-800 backdrop-blur-md rounded-3xl p-8 md:p-10 shadow-2xl relative">
            ...
            <div className="text-center mb-8 border-b border-slate-800/60 pb-6">
              <h1 className="font-sans font-black text-lg tracking-wider text-white uppercase mb-2">
                BIGGS FUNDING SOLUTIONS - ADMIN ACCESS
              </h1>
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3.5 bg-[#0ba5f9] hover:bg-[#008ee3] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all hover:shadow-lg hover:shadow-[#0ba5f9]/20 active:scale-[0.98] disabled:opacity-50 disabled:scale-100 cursor-pointer"
              >
                {isLoading ? "Validating Session..." : "Request Admin Access"}
              </button>
            </form>
          </div>
        </main>
  ...
  return (
    <div className="min-h-screen bg-[#020b24] text-slate-100 font-sans flex flex-col selection:bg-blue-500/30">
      {/* Sleek Administrative Utility Bar */}
      <header className="border-b border-slate-800/80 bg-[#020b24] px-6 py-5 sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          {/* Left Side: Clickable Corporate Logo (Routes home) */}
          <Link
            href="/"
            className="font-sans font-black text-lg text-white tracking-widest leading-none hover:text-[#0ba5f9] transition-colors uppercase shrink-0"
          >
            BIGGS FUNDING SOLUTIONS
          </Link>

          {/* Right Side: Profile Actions Slot */}
          <div className="flex items-center gap-4 shrink-0">
            {/* Elegant Session Status Indicator Pill */}
            ...
            <button
              ...
              disabled={isLoading}
              className="text-[10px] bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-350 hover:text-white font-bold uppercase tracking-widest px-4 py-2 rounded-xl transition-all disabled:opacity-50 cursor-pointer"
            >
        ...
        <div className="flex border-b border-slate-800/80 bg-slate-900/10 rounded-t-2xl">
          <button
            ...
            className={`py-4 px-6 text-xs font-black uppercase tracking-widest border-b-2 transition-all flex items-center gap-2.5 ${
              activeTab === "queue"
                ? "border-[#0ba5f9] text-[#0ba5f9] bg-[#020b24]/40"
                : "border-transparent text-slate-400 hover:text-slate-200 hover:bg-slate-900/10"
            }`}
          >
            Application Queue
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
              activeTab === "queue" ? "bg-[#0ba5f9]/20 text-[#0ba5f9]" : "bg-slate-800 text-slate-400"
            }`}>
              {applications.length}
            </span>
          </button>
          ...
          >
            Message Inbox
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
              activeTab === "inbox" ? "bg-[#0ba5f9]/20 text-[#0ba5f9]" : "bg-slate-800 text-slate-400"
            }`}>
              {messages.length}
            </span>
          </button>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-[#06153b] border border-slate-800/80 rounded-2xl p-5 md:p-6 shadow-lg">
                  ...
                  <span className="text-xl md:text-2xl font-black text-white mt-1 block font-mono">
                    {formatCurrency(totalVolume.toString())}
                  </span>
                </div>
                <div className="bg-[#06153b] border border-slate-800/80 rounded-2xl p-5 md:p-6 shadow-lg">
                  <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <div className="bg-[#06153b] border border-slate-800/80 rounded-2xl p-5 md:p-6 shadow-lg">
                  <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    System Cleanliness Index
                  </span>
                  <span className="inline-flex items-center gap-1.5 text-emerald-450 text-sm md:text-base font-bold mt-1 uppercase tracking-wider">
              ...
              <div className="flex items-center justify-between border-b border-slate-800 pb-4 mt-6">
                <h2 className="text-xs font-black text-[#0ba5f9] tracking-widest uppercase">
                  ACTIVE LEADS AND FUNDING FILES
                </h2>
              </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-4 font-bold text-white uppercase">
                              ...
                              <span className="block font-mono text-[9px] text-slate-500 font-normal mt-0.5 lowercase tracking-normal">
                            ...
                            {/* Requested Amount */}
                            <td className="py-4 px-4 font-bold text-[#0ba5f9] font-mono">
                            ...
                            <td className="py-4 px-4 space-y-0.5 text-xs" onClick={(e) => e.stopPropagation()}>
                              ...
                              <div className="text-slate-450 select-all font-mono text-[10px]">{app.company.user.email}</div>
                              <div className="text-slate-450 font-mono text-[10px]">{app.company.phone}</div>
                            </td>

                            {/* Filing Date */}
                            <td className="py-4 px-4 text-slate-450 font-medium font-mono">
                            ...
                            <td className="py-4 px-4" onClick={(e) => e.stopPropagation()}>
                              <div className="flex items-center gap-2">
                                <select
                                  ...
                                  className={`bg-slate-950 border text-xs font-semibold rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#0ba5f9] cursor-pointer disabled:opacity-50 ${
                                    app.status === "APPROVED" || app.status === "FUNDED"
                                      ? "border-emerald-500/30 text-emerald-400"
                                      : app.status === "IN_REVIEW"
                                      ? "border-blue-500/30 text-[#0ba5f9]"
                                      : app.status === "DENIED"
                                      ? "border-rose-500/30 text-rose-450"
                                ...
                                {isUpdating && (
                                  <span className="h-2.5 w-2.5 rounded-full border border-t-transparent border-[#0ba5f9] animate-spin shrink-0" />
                                )}
                              </div>
                            </td>
                          </tr>
                    ...
                    return (
                      <div
                        ...
                        className={`border rounded-2xl transition-all duration-300 ${
                          isExpanded
                            ? "bg-slate-950/80 border-[#0ba5f9]/40 shadow-lg shadow-[#0ba5f9]/5"
                            : "bg-slate-900/30 border-slate-855 hover:bg-slate-900/50 hover:border-slate-800"
                        }`}
                      >
                        <button
                          onClick={() => setExpandedMessageId(isExpanded ? null : msg.id)}
                          className="w-full text-left p-5 md:p-6 focus:outline-none flex flex-col md:flex-row md:items-center md:justify-between gap-4"
                        >
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs">
                              ...
                              <span className="text-slate-400 font-semibold">
                                Email: <span className="text-slate-200 font-mono select-all">{msg.email}</span>
                              </span>
                              <span className="text-slate-400 font-semibold">
                                Phone: <span className="text-slate-200 font-mono select-all">{msg.phone}</span>
                              </span>
                            </div>
                            <div className="text-xs text-slate-350 mt-1">
                              Subject: <span className="text-slate-200 font-semibold">{msg.subject || "General Inquiry"}</span>
                            </div>
                          </div>
                          <div className="flex items-center justify-between md:justify-end gap-4 shrink-0">
                            <span className="text-[10px] text-slate-455 font-semibold uppercase tracking-wider font-mono">
                            ...
                            <span className={`text-[9px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-xl border transition-all ${
                              isExpanded 
                                ? "bg-[#0ba5f9]/15 border-[#0ba5f9]/30 text-[#0ba5f9]"
                                : "bg-slate-950 border-slate-855 text-slate-450 hover:text-slate-200 hover:border-slate-700"
                            }`}>
                        ...
                        {isExpanded && (
                          <div className="px-5 pb-5 md:px-6 md:pb-6 border-t border-slate-800/60 pt-4 animate-in fade-in slide-in-from-top-1 duration-200 space-y-4">
                            <div className="flex justify-between items-center">
                              ...
                              <button
                                onClick={() => handleCopyContact(msg)}
                                className="text-[9px] font-extrabold bg-[#0ba5f9]/10 border border-[#0ba5f9]/20 hover:bg-[#0ba5f9]/25 text-[#0ba5f9] uppercase tracking-widest px-3.5 py-1.5 rounded-xl transition-all cursor-pointer"
                              >
                                Copy Contact Info
                              </button>
                            </div>
                            <p className="text-xs text-slate-200 leading-relaxed bg-slate-950/60 border border-slate-850 p-4 rounded-xl whitespace-pre-line select-text">
                              {msg.message}
                            </p>
                          </div>
                        )}
      ...
      {/* Premium Underwriting Dossier Modal */}
      {selectedApplication && (
        <div className="fixed inset-0 z-[100] bg-[#020b24]/95 backdrop-blur-md flex items-center justify-center p-4 md:p-6 overflow-y-auto selection:bg-blue-500/30">
          <div className="bg-slate-900/90 border border-slate-800 rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="border-b border-slate-800/80 bg-slate-950/40 px-6 py-5 flex justify-between items-center shrink-0">
              <div className="space-y-1">
                <h2 className="text-sm font-black text-[#0ba5f9] tracking-widest uppercase">
                  BIGGS FUNDING SOLUTIONS - UNDERWRITING DOSSIER
                </h2>
                <p className="text-[9px] text-slate-500 uppercase tracking-widest font-semibold font-mono">
                  Filing: {selectedApplication.id}
                </p>
              </div>
              <button
                onClick={() => setSelectedApplication(null)}
                className="text-[10px] bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-350 hover:text-white font-bold uppercase tracking-widest px-4 py-2 rounded-xl transition-all cursor-pointer"
              >
                Close Dossier
              </button>
            </div>
            ...
            <div className="p-6 md:p-8 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* COLUMN A: Client & Corporate Profile */}
              <div className="space-y-4">
                <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  <h3 className="text-xs font-black text-[#0ba5f9] tracking-widest uppercase">
                    Client & Corporate Profile
                  </h3>
                  <button
                    onClick={() => handleCopyLeadDetails(selectedApplication)}
                    className="text-[9px] font-extrabold bg-[#0ba5f9]/10 border border-[#0ba5f9]/20 hover:bg-[#0ba5f9]/25 text-[#0ba5f9] uppercase tracking-widest px-3 py-1.5 rounded-xl transition-all cursor-pointer"
                  >
                    Copy Lead Details
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    ...
                    <span className="text-sm font-bold text-white mt-1 block">
                      {selectedApplication.company.user.firstName} {selectedApplication.company.user.lastName}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Company Legal Name</span>
                    <span className="text-sm font-bold text-white mt-1 block uppercase truncate" title={selectedApplication.company.legalName}>
                      {selectedApplication.company.legalName}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Phone Number</span>
                    <span className="text-sm font-bold text-white mt-1 block select-all font-mono">
                      {selectedApplication.company.phone}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Corporate Email</span>
                    <span className="text-sm font-bold text-white mt-1 block select-all font-mono truncate" title={selectedApplication.company.user.email}>
                      {selectedApplication.company.user.email}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Annual Revenue</span>
                    <span className="text-sm font-bold text-[#e08b00] mt-1 block font-mono">
                      {formatCurrency(selectedApplication.company.revenueAnnual)}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Time In Business</span>
                    <span className="text-sm font-bold text-white mt-1 block">
                      {selectedApplication.timeInBusiness}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Credit Score Tier</span>
                    <span className="text-sm font-bold text-white mt-1 block">
                      {selectedApplication.creditScoreTier}
                    </span>
                  </div>
                  <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl">
                    <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Requested Funding</span>
                    <span className="text-sm font-bold text-[#0ba5f9] mt-1 block font-mono">
              ...
              {/* COLUMN B: Secured Document Vault */}
              <div className="space-y-4 flex flex-col h-full">
                <h3 className="text-xs font-black text-[#0ba5f9] tracking-widest uppercase border-b border-slate-800 pb-2 shrink-0">
                  Secured Document Vault (Attached Files)
                </h3>
                  ...
                  {selectedApplication.documents.length === 0 ? (
                    ...
                    <div className="space-y-3 flex-grow overflow-y-auto max-h-[280px] pr-2">
                      {selectedApplication.documents.map((doc) => {
                        ...
                        const isExcel = doc.mimeType.toLowerCase().includes("sheet") || 
                                        doc.mimeType.toLowerCase().includes("excel") || 
                                        doc.originalName.endsWith(".xlsx") || 
                                        doc.originalName.endsWith(".xls");
                        ...
                        return (
                          <div
                            key={doc.id}
                            className="flex items-center justify-between bg-slate-950/50 border border-slate-850 p-4 rounded-xl gap-4 hover:border-slate-800 transition-colors"
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="shrink-0">{fileIcon}</div>
                              <div className="min-w-0">
                                <span className="block text-xs font-bold text-white truncate pr-2" title={doc.originalName}>
                                  {doc.originalName}
                                </span>
                            ...
                            <a
                              ...
                              className="shrink-0 text-[10px] font-bold bg-slate-900 border border-slate-800 hover:bg-slate-800 text-[#0ba5f9] px-4 py-2 rounded-xl transition-all cursor-pointer uppercase tracking-wider"
                            >
                              Download File Asset
                            </a>
                          </div>
